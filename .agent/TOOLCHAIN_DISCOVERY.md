...
```
1. Check for package.json           → read "scripts" keys               (Node.js)
2. Check for Makefile              → read target names                 (generic/C)
3. Check for Pipfile / pyproject.toml / requirements.txt → Python project
4. Check for Cargo.toml            → Rust project                      (cargo)
5. Check for go.mod                → Go project                        (go)
6. Check for Gemfile               → Ruby project                      (bundle/rake)
```
...
|------|-----------|----------|------------------|
| **Lint** | `npm run lint`, `make lint`, `golangci-lint run`, `cargo clippy`, `go vet`, `ruff check` | Run linter binary directly if installed | WARN: "No linter detected — lint gate skipped" |
